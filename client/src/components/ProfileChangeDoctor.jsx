...
function parseSlot(raw) {
  ...
  // already {startingTime, endingTime, ...}
...
export default function DoctorProfileChange() {
  ...
  const handleSpecializationChange = (specialization) => {
    setDoctorData((prevData) => {
      const updatedSpecializations = prevData.specializations.includes(
        specialization,
      )
  ...
  const handleSlotChange = (index, field, value) => {
    setDoctorData((prevData) => {
      const updatedSlots = prevData.slots.map((slot, i) =>
  ...
  if (isFetching) {
    return <p className={classes.loadingText}>Loading...</p>;
  }
      ...
      <form className={classes.form} onSubmit={handleSubmit}>
        <div className={classes.formHeader}>
          <h1 className={classes.title}>
            {translatedTexts["Doctor Profile Settings"] ||
              "Doctor Profile Settings"}
          </h1>
        </div>

        <label className={classes.label}>
          {translatedTexts["Qualification"] || "Qualification"}
        </label>
        <input
        ...
        <label className={classes.label}>
          {translatedTexts["Experience"] || "Experience (in years)"}
        </label>
        <input
          className={classes.input}
          type="number"
        ...
        <label className={classes.label}>
          {translatedTexts["Description"] || "Description"}
        </label>
        <textarea
          ...
          onChange={(e) =>
            setDoctorData({ ...doctorData, description: e.target.value })
          }
        />
          ...
          doctorData.slots.map((slot, index) => (
            <div key={slot._id ?? index} className={classes.slotContainer}>
              <input
                ...
                onChange={(e) =>
                  handleSlotChange(index, "startingTime", e.target.value)
                }
                required
              />
              <input
                ...
                onChange={(e) =>
                  handleSlotChange(index, "endingTime", e.target.value)
                }
                required
              />
              <input
